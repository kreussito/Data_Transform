...
| **Reference workbooks** | `Intake_v1.xlsx` · `Intake_Engineering_v1.xlsx` · `Intake_EngineeringCombined_v1.xlsx` · `Intake_FireCat_v1.xlsx` · `Intake_FireEQWind_v1.xlsx` · `Intake_FireCatLosses_v1.xlsx` · `Intake_FireCatFull_v1.xlsx` · `Intake_FireCatFull_Transposed_v1.xlsx` · `Intake_Mexico_v1.xlsx` |
...
**Every dataset may be transposed, not only `01` and `02`.** That was an assumption until
`Intake_FireCatFull_Transposed_v1.xlsx`, which is the complete pack flipped mechanically
by `tools/transpose_pack.py` — nothing retyped, so any difference in the output belongs to
the tool. All nine datasets come out record for record identical, and §10.3, §10.4 and
§10.5 reach the same answers, because they read blocks and never a layout.

**Blocks stacked on a transposed sheet share one label column.** Row-wise, each block has
its own extraction *row*, so scanning it can never pick up a neighbour's labels.
Transposed, three versions of an aggregate put `Zone` three times into one column, and a
scan of the whole column is ambiguous. The band is the mirror of the row-wise rule: a
block's labels lie above its own selector row and below the selector row of the block
before it. Nothing new is declared — the boundary is already in the sheet, in the
`Info_i` markers.
...
**S13 and §10.4.** An unranked suffix sorts last, and two of them sort alphabetically
against each other. For a table nobody reads in order that is a harmless default; for the
growth block it is not, because growth is measured from the *first* version to the *last*
and `at expiry` precedes `at inception` in the alphabet. Get the sequence wrong and the
span is wrong — silently, and plausibly. So §10.4 names any suffix `⟦PERIOD ORDER⟧` does
not rank rather than ordering on it quietly.
...
Nine treaty shapes, generated by `tools/build_v1.py`, `tools/build_intake.py` and
...
| `Intake_FireCatFull_Transposed_v1.xlsx` | Fire, Earthquake, Windstorm | **the same pack on its side** — all nine datasets transposed, generated from the row-wise one and verified identical, §6.2 |
